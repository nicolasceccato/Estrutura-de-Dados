...
void concatena_listas (Lista *a, Lista *b){
    Elemento *auxiliar = a->cabeca;
    if (auxiliar != NULL){
        while (auxiliar->proximo != NULL){
            auxiliar = auxiliar->proximo;
        }
        auxiliar->proximo = b->cabeca;
...
Lista busca_todos( Lista l, void *info, int (*compara)(void*,void*) ){
    Lista a;
    inicializa_lista(&a, 4);
    if(lista_vazia(l)){
        return a;
    }
    Elemento *auxiliar = l.cabeca;
    int cont = 0;
    while(auxiliar != NULL){
        int x = compara(auxiliar->info, info);
        if (x == 0){
            insere_ordem(&a, &cont, compara);
        }
        auxiliar = auxiliar->proximo;
        cont++;
    }
